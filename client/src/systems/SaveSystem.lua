...
function SaveSystem:serializeGameScene(gameScene)
    return {
        ...
        isDead = gameScene.isDead or false,
        targetFloor = gameScene.targetFloor or gameScene.currentFloor + 1
    }
end
...
function SaveSystem:deserializeGameScene(gameScene, data)
    ...
    gameScene.isDead = data.isDead or false
    gameScene.targetFloor = data.targetFloor or (data.currentFloor or 1) + 1
    gameScene.currentMonster = nil  -- Всегда сбрасываем монстра при загрузке
    
    -- Если прогресс максимальный - показываем кнопку перехода
    if gameScene.killedMonsters >= 3 then
        gameScene.showAdvanceButton = true
        gameScene.canAdvance = true
    end
end
...
function SaveSystem:deserializeItem(data)
    local item = {
        ...
        rarity = data.rarity,
        name = data.name,
        baseAttack = data.baseAttack or 0,
        baseDefense = data.baseDefense or 0,
        baseHP = data.baseHP or 0,
        baseAttackSpeed = data.baseAttackSpeed or 0,
        bonusAttack = data.bonusAttack or 0,
        bonusDefense = data.bonusDefense or 0,
        bonusHP = data.bonusHP or 0,
        bonusAttackSpeed = data.bonusAttackSpeed or 0,
