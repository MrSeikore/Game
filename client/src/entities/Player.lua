...
function Player:new()
    local o = {
        ...
        attackSpeed = 1.0,  -- Базовый ATS (голые руки)
...
function Player:recalculateStats()
    ...
    self.attackSpeed = 1.0  -- Базовый ATS без оружия (голые руки)
    ...
    -- Add equipment bonuses (ИСПРАВЛЕННАЯ ЛОГИКА)
    for slot, item in pairs(self.equipment) do
        if item then
            -- Основные статы (суммируем базовые + бонусы напрямую)
            self.maxHp = self.maxHp + (item.baseHP or 0) + (item.bonusHP or 0)
            self.attack = self.attack + (item.baseAttack or 0) + (item.bonusAttack or 0)
            self.defense = self.defense + (item.baseDefense or 0) + (item.bonusDefense or 0)
            
            -- Attack Speed: ЗАМЕНЯЕМ базовый ATS на ATS оружия + бонусы от аффиксов
            if slot == "weapon" then
                self.attackSpeed = (item.baseAttackSpeed or 0) + (item.bonusAttackSpeed or 0)
            else
                -- Для не-оружия добавляем только бонусы от аффиксов
                self.attackSpeed = self.attackSpeed + (item.bonusAttackSpeed or 0)
            end
...
function Player:getEquipmentStats()
    ...
    for slot, item in pairs(self.equipment) do
        if item then
            stats.hp = stats.hp + item:getTotalHP()
            stats.attack = stats.attack + item:getTotalAttack()
            stats.defense = stats.defense + item:getTotalDefense()
        end
    end
