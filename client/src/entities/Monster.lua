...
function Monster:new(floorLevel)
    local o = {
        ...
        maxHp = math.floor((40 + (floorLevel * 10)) * 0.6),
        hp = math.floor((40 + (floorLevel * 10)) * 0.6),
        attackDamage = math.floor((5 + floorLevel)),
        defense = math.floor(floorLevel),
        expValue = 20 + floorLevel * 5,  -- Опыт без изменений
        attackSpeed = 1.0,
        lastAttackTime = 0,
...
function Monster:attackPlayer(player, currentTime)
    if self:canAttack(currentTime) then
        local damage = math.max(1, self.attackDamage - math.floor(player.defense / 2))
